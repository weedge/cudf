...
if buildAll || hasArg cudf; then
    ...
    CMAKE_ARGS="-DCMAKE_PREFIX_PATH=${INSTALL_PREFIX} -DCMAKE_LIBRARY_PATH=${LIBCUDF_BUILD_DIR} -DCMAKE_CUDA_ARCHITECTURES=${CUDF_CMAKE_CUDA_ARCHITECTURES} ${EXTRA_CMAKE_ARGS}" \
...
if hasArg cudf_kafka; then
    ...
    CMAKE_ARGS="-DCMAKE_LIBRARY_PATH=${LIBCUDF_BUILD_DIR}" \
        python -m pip install --no-build-isolation --no-deps .
fi
